...
void easeMeWithColor(CRGB lightColor, int easeOutValue, int easeInValue, int lerpValue ) {
  static uint8_t easeOutVal = easeOutValue;
  static uint8_t easeInVal  = easeInValue;
  static uint8_t lerpVal    = lerpValue;
  easeOutVal = ease8InOutQuad(easeInVal);
  easeInVal++;
  lerpVal = lerp8by8(0, NUM_LEDS, easeOutVal);
  leds[lerpVal] = lightColor;
  fadeToBlackBy(leds, NUM_LEDS, 8);                     // 8 bit, 1 = slow, 255 = fast
  FastLED.show();
  delay(5);
}
...
void loop() {
  ...
  if ( readingIn > 500 ) {
    ...
    easeMeWithColor(CRGB::Aqua, 2000, 1000, 18);
  }
